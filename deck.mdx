...
```ts 8:13 subtitle="😱 What does that even ? 🤔"
interface AsyncUser {
  isLoading: boolean
  didLoad: boolean
  data?: User
...
```diff 2:3 title="😒 No single source of truth 👎"
...
```jsx title="😎 Single source of truth 👍"
...
```diff 2[7:29],6[7:32],10[7:47],14[7:46]
...
<Multiline lines={['🚢 📦', 'Ok, now is all good, right?']} />
...
<Multiline lines={[<span>Is it <i>tho</i> ?</span>, '🤔']} />

---

<CodeSurfer>

```ts 13:23 subtitle="🤷 Again Joe? 🤦♂️"
type Status =
  | 'idle'
  | 'loading'
  | 'success'
...
```diff 14:16,20:22 subtitle="🤷 Again Joe? 🤦♂️"
...
```diff 2[5:20],3[5:23],4[5:32],5[5:37]

```

```diff 4[25:31],5[25:36] subtitle="No more optional values 🤯"
...
```ts subtitle="What do we want to do for each case? 🤨"
type AsyncData<T> =
  | {status: 'idle'}
  | {status: 'loading'}
  | {status: 'success', data: T}
...
const LoadingUser: React.FC<AsyncData<User>> = (asyncUser) =>
