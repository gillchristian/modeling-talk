...
```jsx 2[7:29],6[7:32],10[7:47],14[7:46] title="😎 Single source of truth 👍"
...
<Multiline lines={[<span>But, is it <i>tho</i> ?</span>, '🤔']} />
...
<Multiline
  Elem="h3"
  lines={['🙄 Maybe ... 👀', 'We can still improve it?']}
/>
...
```ts title="What do we want to do for each case? 🤨"
type AsyncData<T> =
  | {status: 'idle'}
  | {status: 'loading'}
  | {status: 'success', data: T}
